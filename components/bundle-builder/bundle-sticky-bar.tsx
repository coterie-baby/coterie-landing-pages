...
import { useBundleSelector, WIPES_PRODUCTS, SKINCARE_ITEMS } from './bundle-selector';
...
export default function BundleStickyBar() {
  ...
  const hasItems = !!selectedSize || !!wipesConfig || selectedSkincareIndices.length > 0;
      ...
      {showItems && hasItems && (
        <div className="max-w-lg mx-auto px-4 pt-4 pb-2 space-y-3 border-b border-gray-100 shadow-[0_-8px_24px_rgba(0,0,0,0.08)]">
            ...
            <div className="flex items-center gap-3">
              {diaperImageUrl && (
                <div className="relative w-12 h-12 flex-shrink-0 rounded-lg overflow-hidden bg-gray-100">
                  <Image src={diaperImageUrl} alt="The Diaper" fill className="object-cover" sizes="48px" />
                </div>
              )}
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-gray-900">The Diaper — {getSizeLabel(selectedSize)}</p>
                <p className="text-xs text-gray-500">{SIZE_CONFIGS[selectedSize].count} diapers/delivery</p>
              </div>
              <p className="text-sm font-semibold text-gray-900">${diaperPrice.toFixed(2)}</p>
            </div>
          )}
          ...
          {wipesConfig && (
            <div className="flex items-center gap-3">
              <div className="relative w-12 h-12 flex-shrink-0 rounded-lg overflow-hidden bg-gray-100">
                <Image src={wipesConfig.image} alt={wipesConfig.name} fill className="object-cover" sizes="48px" />
              </div>
              <div className="flex-1 min-w-0">
              ...
              <p className="text-sm font-semibold text-gray-900">${wipesPrice.toFixed(2)}</p>
            </div>
          )}
        ...
        <div className="flex items-stretch gap-2">
          ...
          {hasItems && (
            ...
            >
              ...
              <span>View your<br />bundle items</span>
            </button>
          )}
        </div>
      </div>
